...
signinForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    const response = await fetch(
      "http://localhost:8081/Crop-Monitor/api/v1/auth/signin",
      {
        method: "POST",
        headers: {
    ...
    // Handle the response
    if (response.ok) {
      responseMessage.textContent = "Signin successful!";
      ...
      setTimeout(() => {
        window.location.href = "/pages/mainPage.html"; // Adjust the redirect page as needed
      }, 2000);
    } else {
